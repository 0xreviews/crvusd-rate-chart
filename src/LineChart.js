...
export default function LineChart({
  ...
  useEffect(() => {
            ...
            beforeEvent(chart, args, pluginOptions) {
              ...
              if (event.type === "mouseout") {
                ...
                setTimeout(() => {
                  ...
                  const rateTooltip = document.querySelector(
                    `#${type}TooltipId`
                  );
                  const left =
    ...
    if ( !chart.tooltip.getActiveElements().length && defaultTooltipIndex > -1) {
      ...
      if (type === 'fractionRate') {
        title = `fraction: ${(data.labels[defaultTooltipIndex] * 100).toFixed(2)}%`;
      ...
      const { ctx, chartArea: { top, bottom, left, right }, scales: {x, y}, canvas: {offsetLeft}} = chart;
      
      externalTooltipHandler({ chart, tooltip: chart.tooltip }, `${type}TooltipId`, title, value, tooltipHead2, defaultTooltipIndex);
      const rateTooltip = document.querySelector(`#${type}TooltipId`);
      rateTooltip.style.left = `${offsetLeft + x.getPixelForValue(defaultTooltipIndex)}px`;
  ...
  return (
    <div className="chart-container">
      <canvas id={`${type}ChartId`} ref={chartRef} width={600} height={320}></canvas>
    </div>
  );
}
